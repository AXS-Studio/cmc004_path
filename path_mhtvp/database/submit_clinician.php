...
function addNewUser($SHSCID,$FirstName,$LastName,$Email){
   global $mysqli;
   $admin = 0; //New clinicians are NOT set to admin status
   $result = true; //Track mysql errors throughout function
   
   //--Add new entry for Clinician table using prepared statement--
   if ($stmt =  $mysqli->prepare("INSERT INTO `Clinician`(`SHSCID`, `FirstName`, `LastName`, `Email`, `Admin`) VALUES (?,?,?,?,?)")){
	$stmt->bind_param('ssssi',$SHSCID, $FirstName,$LastName, $Email, $admin );
	$result = $stmt->execute();
	if (!$result)
   	return $mysqli->error; //just for reference: Duplicate key error is $mysqli->errrno == 1062
   }
...
function updateUser($OldSHSCID,$SHSCID,$FirstName,$LastName,$Email){
	...
	if ($stmt =  $mysqli->prepare("UPDATE `Clinician` SET `SHSCID`=?,`FirstName`=?,`LastName`=?,`Email`=?
			 WHERE `SHSCID` = ?")){
		$stmt->bind_param('sssss',$SHSCID,$FirstName,$LastName,$Email,$OldSHSCID);
		$result = $stmt->execute();
		if (!$result)
   		return $mysqli->error; //just for reference: Duplicate key error is $mysqli->errrno == 1062
	}

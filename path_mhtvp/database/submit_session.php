...
if ($action == 'new' || $action == 'save'){
	...
	*/
	...
	$q = "INSERT INTO `Sessions_PATH`(`PatientID`, `ClinicianID`, `Date`, `SessionName`, `Settings`) VALUES (?,?,?,?,?)";
	if ($stmt = $mysqli->prepare($q)){
		$stmt->bind_param('sssss', $patientID, $clinicianID, $date, $sessionName, $settingsString );	// Bind our param as string
...
//Action: Rename session
	...
	global $mysqli;
	$q = "UPDATE `Sessions_PATH` SET `SessionName`= '$newSessionName'
		WHERE `PatientID` = '$patientID'
		AND `ClinicianID` = '$clinicianID'
		AND `SessionName` = '$sessionName'";

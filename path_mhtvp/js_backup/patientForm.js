var PatientForm = (function() {
	...
	function sendEmail(email, password, firstName) {	// ALERT: Stopped here.
		...
		// window.open('mailto:' + email + '?cc=pathadmin@path.sunnybrook.ca&subject=Your Mental Health Telemetry Account&body=' + accountInformation);
		$('#rc').html(currentRc + '.<br>\
			<a href="mailto:' + email + '?cc=pathadmin@path.sunnybrook.ca&subject=Your Mental Health Telemetry Account&body=' + accountInformation + '" title="Send ' + firstName + ' an email">Send ' + firstName + ' an email.</a>');
	}

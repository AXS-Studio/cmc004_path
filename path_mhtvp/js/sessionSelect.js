				...
				switch (action) {
					...
					default : 
						$('#cfgGraphs').html('<div class="svgWrap" id="svgParent" style="display: none;">\
							...
							<p class="lbl lblCn">Clinician Notes</p>\
							<p class="lbl lblPc">Patient Comments</p>\
						</div>\
						<!-- end svgWrap -->\
	...
	function ssStart() {
		$('#cfg').append('<form id="ssForm" action="#">\
				<fieldset>\
					...
					<select name="session" id="sessSelect" onchange="SessionSelect.ssGo();">\
					...
					<!-- end sessSelect -->\
					&nbsp;<a href="#" title="Save PDF" class="img btnPrintPdf" id="ppdf"><span>Save PDF</span></a>\
				</fieldset>\
			</form>');
		...
		if (sessions.data.length > 0) {
			for (var i = 0; i < sessions.data.length; i++) {
				$('#sessSelect').append('<option value="session-' + sessions.data[i].value + '">' + sessions.data[i].name + '</option>');
			}
		}
